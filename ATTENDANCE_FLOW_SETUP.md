...
## Current Live Attendance Architecture

- Teacher assignment is the exact identity: Teacher + Class/Section + Subject + Semester + Session.
- `teacherSubjectAssignments/{assignmentId}` is the immutable link used by schedules and live attendance sessions.
- Students are enrolled against the exact teacher assignment for their class/section.
- Teacher starts a live session; its QR payload is stored in `attendanceSessions`.
- Students receive the active session in realtime and see the same QR visually. They do NOT scan it.
- Student presses `Save Attendance`; the server-side Firestore validation checks account, session, date/expiry, class identity, schedule, teacher UID, exact teacher assignment, subject enrollment and duplicate attendance before saving.
