...
class StudentSubjectAssignmentRepository(
     ...
     */
    private suspend fun getExactTeacherAssignments(student: Student): Map<String, Pair<String, Pair<String, String>>> {
        return teacherAssignmentsRef.get().await().documents.mapNotNull { d ->
            val classId = d.getString("classId").orEmpty()
            val subjectId = d.getString("subjectId").orEmpty()
            val semester = numberField(d.get("semester"))
            val session = d.getString("session").orEmpty().trim()
            val section = d.getString("section").orEmpty().trim()
            if (classId == student.classId && subjectId.isNotBlank() && semester == student.semester &&
                session.equals(student.session.trim(), true) && section.equals(student.section.trim(), true)) {
                subjectId to Pair(d.id, Pair(d.getString("teacherId").orEmpty(), d.getString("teacherName").orEmpty()))
            } else null
        }.toMap()
    }
    ...
    suspend fun getSubjectsForStudent(student: Student): List<Subject> {
        ...
        val exactTeacherAssignments = getExactTeacherAssignments(student)
        ...
        return subjectsRef.get().await().documents.mapNotNull { d ->
            ...
            if (d.id !in exactTeacherAssignments) {
                return@mapNotNull null
            }
    ...
    suspend fun saveAssignments(
        ...
        return try {
            ...
            val missingTeacherAssignment = selectedSubjectIds.firstOrNull { it !in exactTeacherAssignments }
            if (missingTeacherAssignment != null) {
                return Result.Error("Selected subject is not assigned to a teacher for this exact class, semester, session and section.")
            }
                ...
                .forEach { subject ->
                    ...
                    val model =
                        StudentSubjectAssignment(
                            ...
                            teacherAssignmentId =
                                exactTeacherAssignments[subject.subjectId]!!.first,

                            teacherId =
                                exactTeacherAssignments[subject.subjectId]!!.second.first,

                            teacherName =
                                exactTeacherAssignments[subject.subjectId]!!.second.second,
            ...
            ActivityLogHelper.log(
                type = Type.STUDENT_SUBJECT_ASSIGNED,
                title = "Student Subjects Assigned",
                description = "${student.fullName}: ${selectedSubjectIds.size} subject(s) enrolled for ${student.programName}, Semester ${student.semester}, Section ${student.section}"
            )
