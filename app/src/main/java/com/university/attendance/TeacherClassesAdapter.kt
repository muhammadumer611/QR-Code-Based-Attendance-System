...
class TeacherClassesAdapter(
    ...
    inner class ClassViewHolder(
        ...
        fun bind(
            ...
            semester.text =
                "Semester: ${subject.semester}  •  Session: ${subject.sessionName.ifBlank { "-" }}"

            program.text = if (subject.className.isNotBlank() || subject.section.isNotBlank()) {
                "Class: ${subject.className.ifBlank { subject.programName }}  •  Section ${subject.section.ifBlank { "-" }}"
            } else {
                "Program: ${subject.programName}"
            }
