...
class TeacherAttendanceRepository(private val firestore: FirebaseFirestore = FirebaseFirestore.getInstance()) {
    ...
    suspend fun createSession(schedule: ClassSchedule): AttendanceSession {
        ...
        val exactAssignmentDoc = assignmentsRef.whereEqualTo("teacherId", teacherId).get().await().documents.firstOrNull { doc ->
            doc.getString("classId") == schedule.classId &&
                doc.getString("subjectId") == schedule.subjectId &&
                (doc.getLong("semester")?.toInt() ?: doc.getString("semester")?.toIntOrNull()) == schedule.semester &&
                doc.getString("session").orEmpty().trim().equals(schedule.session.trim(), true) &&
                doc.getString("section").orEmpty().trim().equals(schedule.section.trim(), true)
        }
        val assignmentId = exactAssignmentDoc?.id.orEmpty()
        if (exactAssignmentDoc == null) throw Exception("This teacher is not assigned to this exact class, subject, semester, session and section.")
        if (schedule.assignmentId.isNotBlank() && schedule.assignmentId != assignmentId) {
            throw Exception("This schedule is linked to a different teacher assignment.")
        }
        ...
        if (existing != null) return AttendanceSession(
            ...
            teacherAuthUid = existing.getString("teacherAuthUid").orEmpty(), assignmentId = existing.getString("assignmentId").orEmpty(), teacherName = existing.getString("teacherName").orEmpty(),
        ...
        val data = mapOf(
            "scheduleId" to schedule.scheduleId, "assignmentId" to assignmentId, "teacherId" to teacherId, "teacherAuthUid" to uid, "teacherName" to schedule.teacherName,
        ...
        return AttendanceSession(sessionId = sessionId, scheduleId = schedule.scheduleId, assignmentId = assignmentId, teacherId = teacherId, teacherAuthUid = uid,
