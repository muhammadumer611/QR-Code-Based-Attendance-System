...
class ActivityAddStudent : AppCompatActivity() {
    ...
    private fun setupDropdowns() {
        ...
        // Every dropdown field must open its list on a single tap instead of
        // requiring the user to type first (AutoCompleteTextView's default
        // behaviour). This keeps Program / Section / Semester consistent with
        // every other dropdown in the app (e.g. Teacher Subject Assignment).
        binding.etProgramName.setOnClickListener {
            binding.etProgramName.showDropDown()
        }

        binding.etSection.setOnClickListener {
            binding.etSection.showDropDown()
        }
