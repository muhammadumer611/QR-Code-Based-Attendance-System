...
class ActivityAddTeacher : AppCompatActivity() {
    ...
    private fun setupDesignationDropdown() {
        ...
        // inputType=none + focusable=false in XML means every tap is a pure
        // click (no focus-steal on first tap), so this fires reliably.
    ...
    private fun observeDepartments() {
        viewModel.departments.observe(this) { departments ->
            ...
            // Safety net: if focus is ever regained programmatically
            // (e.g. after rotation), still pop the dropdown.
            binding.etDepartment.setOnFocusChangeListener { _, hasFocus ->
                if (hasFocus) binding.etDepartment.showDropDown()
            }
