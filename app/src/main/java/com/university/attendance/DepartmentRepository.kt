 ...
 * Handles all Firestore CRUD operations for the "departments" collection.
 *
 * UPDATED: after a successful addDepartment(), logs an ActivityLog +
 * Notification via ActivityLogHelper. Update/Delete are NOT logged here
 * per current scope (only creation events are logged across all
 * repositories for now, to keep the feed focused on additions).
 */
class DepartmentRepository(
    private val firestore: FirebaseFirestore = FirebaseFirestore.getInstance()
) {
    ...
    suspend fun addDepartment(name: String): OpResult {
        return try {
            ...
            ActivityLogHelper.log(
                type = Type.DEPARTMENT_ADDED,
                title = "Department Added",
                description = "${name.trim()} department was created"
            )
